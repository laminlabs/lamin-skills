...
- **Run**: an execution. Each harness conversation gets one Run of your harness's fixed Transform (the **agent run**) per LaminDB instance. If the user sends a follow-up after you already ran `lamin finish`, run `lamin track <agent>` again: it resumes that same agent Run using the harness session ID (or Cursor's branch-local state) instead of creating a duplicate, and the next `lamin finish` replaces its report with the complete conversation. Every script you write self-tracks its *own* Run the instant it executes, linked back to the agent run via `initiated_by_run` — see "Self-tracking scripts" below. You never construct the script's Transform/Run by hand from outside.
